...
import { AnnouncementForm, type AnnouncementEdit } from "./AnnouncementForm";
...
export type CommentItem = {
  ...
  is_approved: boolean;
  created_at: string;
  announcements: { title_am?: string; slug?: string } | null;
};
...
export function AnnouncementsClient({
  ...
  const [editing, setEditing] = useState<AnnouncementEdit | null>(null);
  ...
  return (
    <div>
      ...
      <p className="text-sm text-[var(--foreground)]/60 mt-1 amharic">
        ፍጠር · አርትዕ · አትም / ደብቅ · ሰርዝ · ምስል · አስተያየት
      </p>

      <div className="mt-8 grid gap-8 lg:grid-cols-2">
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6">
          <h2 className="font-semibold amharic mb-4">
            {editing ? "ጉዳይ አርትዕ" : "አዲስ ወቅታዊ ጉዳይ"}
          </h2>
          <AnnouncementForm
            key={editing?.id ?? "new"}
            edit={editing}
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold amharic">
                      {c.author_name}
                    </p>
                    {c.author_email && (
                    ...
                    <p className="mt-2 text-sm amharic leading-relaxed">
                      {c.body}
                    </p>
