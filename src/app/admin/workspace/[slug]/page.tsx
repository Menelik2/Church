...
export default async function DepartmentWorkspacePage({
  ...
  return (
    <div className="pb-12">
      <Link href="/admin/workspace" className="inline-flex items-center gap-1 text-sm text-[var(--primary)] amharic mb-4">
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 mb-6">
        ...
        <h1 className="text-2xl font-bold amharic text-[var(--primary)] mt-1">{ws.title_am}</h1>
        <p className="mt-2 text-sm amharic text-[var(--foreground)]/70 leading-relaxed">{ws.description_am}</p>
      </div>
      ...
      {activeTab === "overview" && (
        <div className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            {ws.actions.map((a) => (
              <Link key={a.href + a.label_am} href={a.href}
                ...
                <span className="font-semibold amharic text-[var(--primary)]">{a.label_am}</span>
                <ExternalLink className="h-4 w-4 text-[var(--foreground)]/30" />
              </Link>
            ))}
          </div>
          <div className="rounded-2xl border border-[var(--border)] p-4">
            <p className="text-xs font-semibold amharic text-[var(--foreground)]/50 mb-2">የቅርብ ተግባራት</p>
      ...
      {activeTab === "finance" && <FinancePanel initial={(finance as never) ?? []} />}
      {activeTab === "inventory" && <InventoryPanel initial={(inventory as never) ?? []} />}
      {(activeTab === "attendance" || activeTab === "choir") && (
        <RecordPanel departmentCode={slug} recordType="attendance"
          title={activeTab === "choir" ? "የመዝሙር ልምምድ መዝገብ" : "የመገኘት መዝገብ"}
          initial={(records ?? []).filter((r) => r.record_type === "attendance")} />
      )}
      {activeTab === "correspondence" && (
        <RecordPanel departmentCode={slug} recordType="correspondence" title="ደብዳቤና ማስታወሻ"
          initial={(records ?? []).filter((r) => r.record_type === "correspondence")} />
      )}
      {activeTab === "media" && (
        <RecordPanel departmentCode={slug} recordType="media" title="ሚዲያ መዝገብ"
          initial={(records ?? []).filter((r) => r.record_type === "media")} />
      )}
      {activeTab === "charity" && (
        <RecordPanel departmentCode={slug} recordType="event" title="የበጎ አድራጎት ፕሮጀክቶች"
