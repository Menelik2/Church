...
export default async function AdminServantsPage() {
  ...
  const { data: servants } = await supabase.from("servants").select("*").order("created_at", { ascending: false }).limit(100);
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {(!servants || servants.length === 0) && (
              <tr>
                <td colSpan={4} className="px-4 py-8 text-center text-[var(--foreground)]/50">መዝገብ ባዶ ነው።</td>
              </tr>
            )}
          </tbody>
        </table>
