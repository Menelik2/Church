...
import { UserPlus, Heart, AlertTriangle, Calendar, Users, Wallet, FileText, Vote } from "lucide-react";
...
export default async function OperationsHubPage() {
  ...
  const [
    ...
    supabase.from("election_cycles").select("*", { count: "exact", head: true }).neq("status", "completed").neq("status", "cancelled"),
  ]);
  ...
  const cards = [
    ...
    { href: "/admin/operations/contributions", label: "ወርሃዊ መዋጮ", value: "—", icon: Wallet, hint: "አንቀጽ 12/14" },
    { href: "/admin/operations/reports", label: "የክፍል ሪፖርቶች", value: "—", icon: FileText, hint: "ሩብ ዓመት" },
    { href: "/admin/operations/elections", label: "ምርጫ", value: openElections ?? 0, icon: Vote, hint: "አንቀጽ 10 · 2 ዓመት" },
  ];
