  ...
  return (
    <div className="bg-cross-pattern overflow-x-hidden pb-28 lg:pb-0">
            ...
            {primaryActions.map((a, i) => {
              ...
              return (
                <motion.div
                  ...
                  transition={{ delay: 0.28 + i * 0.1, type: "spring", stiffness: 150, damping: 16 }}
                  whileTap={{ scale: 0.96 }}
                  whileHover={{ y: -4 }}
                >
                  <RippleLink
                  ...
                  >
                    ...
                    <span className="min-w-0 text-left">
                      <span className={a.tone === "primary" ? "block text-[13px] font-bold leading-tight amharic text-[var(--primary)] sm:text-sm" : "block text-[13px] font-bold leading-tight amharic text-[var(--color-charcoal)] sm:text-sm"}>
                        {a.title}
                      </span>
          ...
          <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-5 sm:gap-3.5">
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ type: "spring", stiffness: 120, damping: 18 }} className="col-span-2 relative h-40 overflow-hidden rounded-2xl border border-[var(--color-gold-300)]/35 shadow-lg sm:col-span-3 sm:h-52 sm:rounded-3xl">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4">
                <p className="text-[11px] font-semibold text-white amharic sm:text-sm">ቤተ ክርስቲያን — አየር እይታ</p>
      ...
      {announcements.length > 0 && (
        <section className="mx-auto max-w-7xl px-4 pt-10 sm:px-6 sm:pt-12 lg:px-8">
          <div className="mb-4 flex items-end justify-between gap-3">
            ...
            <Link href="/announcements" className="flex shrink-0 items-center gap-0.5 text-xs font-semibold text-[var(--primary)] amharic">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {announcements.slice(0, 6).map((a, i) => (
              <motion.div key={a.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05, type: "spring", stiffness: 140, damping: 18 }}>
                <Link href="/announcements" className="hover-card block h-full rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {a.is_featured && (
                      <span className="text-[10px] rounded-full bg-[var(--color-gold-100)] text-[var(--color-gold-800)] px-2 py-0.5">ባህሪይ</span>
                    )}
                  </div>
                  ...
                  <time className="mt-2 block text-[10px] text-[var(--foreground)]/40">
      ...
      <section className="mx-auto max-w-7xl px-4 pt-10 sm:px-6 sm:pt-12 lg:px-8">
        ...
        <p className="mt-3 flex items-center justify-center gap-1.5 text-[11px] text-[var(--foreground)]/45 amharic"><MapPin className="h-3.5 w-3.5" />ደብረ ሰላም በዓለ እግዚአብሔር · ባሕር ዳር</p>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 pt-10 sm:px-6 sm:pt-12 lg:px-8">
        <div className="mb-4 flex items-end justify-between gap-3">
          ...
          <Link href="/programs" className="flex shrink-0 items-center gap-0.5 text-xs font-semibold text-[var(--primary)] amharic">ሁሉም <ChevronRight className="h-3.5 w-3.5" /></Link>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {PROGRAMS.slice(0, 3).map((p, i) => (
            <motion.div key={p.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06, type: "spring", stiffness: 140, damping: 18 }}>
              <RippleLink href={p.related_href || "/programs"} color="primary" className="hover-card block h-full rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4 shadow-sm">
                <span className="inline-flex h-7 min-w-7 items-center justify-center rounded-lg bg-[var(--primary)]/10 px-2 text-xs font-bold text-[var(--primary)]">{p.order}</span>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12 lg:px-8">
        ...
        <div className="-mx-4 flex gap-3 overflow-x-auto px-4 pb-2 snap-x snap-mandatory scrollbar-none sm:mx-0 sm:grid sm:grid-cols-3 sm:gap-3.5 sm:overflow-visible sm:px-0">
          {DEPARTMENTS.slice(0, 6).map((d, i) => (
            <motion.div key={d.slug} initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05, type: "spring", stiffness: 140, damping: 18 }} className="w-[78%] max-w-[280px] shrink-0 snap-start sm:w-auto sm:max-w-none">
              <RippleLink href={`/departments/${d.slug}`} color="gold" className="hover-card block h-full rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4 shadow-sm">
                ...
                {d.title_en && <p className="mt-1 line-clamp-2 text-[11px] leading-snug text-[var(--foreground)]/50">{d.title_en}</p>}
              </RippleLink>
            </motion.div>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-4 sm:grid-cols-2">
          <RippleLink href="/announcements" color="primary" className="hover-card flex items-start gap-4 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 shadow-sm">
            <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[var(--primary)]/10 text-[var(--primary)]"><Megaphone className="h-6 w-6" /></span>
            <span>
              <span className="block text-base font-bold amharic text-[var(--primary)]">ወቅታዊ ጉዳዮች</span>
              <span className="mt-1 block text-sm text-[var(--foreground)]/60 amharic">የቅርብ ጊዜ ማስታወቂያዎችና ዜናዎች</span>
            </span>
          </RippleLink>
          <RippleLink href="/rules" color="gold" className="hover-card flex items-start gap-4 rounded-2xl border border-[var(--color-gold-300)]/40 bg-[var(--card)] p-5 shadow-sm">
            <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[var(--color-gold-100)] text-[var(--color-gold-700)]"><FileText className="h-6 w-6" /></span>
            <span>
        ...
        <div className="mt-4">
          <RippleLink href="/pdf" color="primary" className="hover-card flex items-center justify-between gap-3 rounded-2xl border border-dashed border-[var(--border)] bg-[var(--muted)]/40 px-5 py-4">
